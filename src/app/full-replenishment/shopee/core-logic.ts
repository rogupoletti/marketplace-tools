...
export function processProduct(
    ...
    // Cap suggestion by Max Allowed (Agendamento)
    const maxPermitida = produto.qtdeMaxPermitida ?? Infinity;
    sugestaoReposicao = Math.min(sugestaoReposicao, maxPermitida);

    let status: StatusReposicao = "ATIVO / OK";

    const shopeeItem = String(produto.shopeeItemId || "").trim();
    const shopeeModel = String(produto.shopeeModelId || "").trim();
    const isShopeeIdEmpty = (val: string) => !val || val === "0" || val === "";
    ...
    if (!isAtivo) {
        status = "INATIVO";
        sugestaoReposicao = 0;
    } else if (isShopeeIdEmpty(shopeeItem)) {
    ...
    return {
        ...
        numCaixas,
        emTransf,
        qtdeMaxPermitida: maxPermitida === Infinity ? 0 : maxPermitida, // For display purposes, show 0 if not set or actually 0
        overrides,
        status
    };
}
