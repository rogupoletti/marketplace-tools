...
const COLUMN_LABELS: Record<string, string> = {
    ...
    qtdeMaxPermitida: "Qtde Máx Permitida",
    asp: "ASP (R$)",
    markup: "Mark up",
};
            ...
            <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between shrink-0 bg-gray-50/50">
                <div className="flex items-center gap-4">
                    <div className="relative w-72">
                        <input
                            type="text"
                            placeholder="Buscar SKU, Descrição ou ID..."
                            value={filtros.busca}
...
function renderCell(
    ...
    switch (colId) {
        ...
        case 'qtdeMaxPermitida':
            return <span className={`text-xs font-bold ${item.qtdeMaxPermitida > 0 ? 'text-orange-600' : 'text-gray-400'}`}>{item.qtdeMaxPermitida > 0 ? formatNumber(item.qtdeMaxPermitida, 0) : '—'}</span>;
        case 'asp':
